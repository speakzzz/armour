...
# This version includes an improved UI for complex settings.
...
namespace eval ::arm::web {
    ...
    proc update_list_counts_coro {minute hour day month weekday} { ::arm::coroexec ::arm::web::update_list_counts $minute $hour $day $month $weekday }
    ...
    proc accept {sock addr p} { variable sessions; fconfigure $sock -buffering line -translation lf; if {[eof $sock] || [catch {gets $sock request_line}]} { catch {close $sock}; return }; set headers [dict create]; while {[gets $sock line] > 0 && $line ne "\r"} { if {[regexp {^([^:]+): (.*)} $line -> key value]} { dict set headers [string trim $key] [string trim $value] } }; lassign $request_line method request_uri version; set query_params [dict create]; if {[string first "?" $request_uri] != -1} { lassign [split $request_uri "?"] path query_string; foreach pair [split $query_string &] { lassign [split $pair =] key value; dict set query_params [url_decode $key] [url_decode $value] } } else { set path $request_uri }; set page 1; if {[dict exists $query_params page]} { set page_val [dict get $query_params page]; if {[string is integer -strict $page_val] && $page_val > 0} { set page $page_val } }; set user ""; if {[dict exists $headers Cookie]} { set cookie_str [dict get $headers Cookie]; if {[regexp {session_id=([^; ]+)} $cookie_str -> session_id] && [dict exists $sessions $session_id]} { set user [dict get $sessions $session_id]; if {[::arm::userdb:get:level $user *] < [::arm::cfg:get web:level]} { set user "" } } }; set post_data ""; if {$method eq "POST"} { set content_length 0; if {[dict exists $headers "Content-Length"]} { set content_length [dict get $headers "Content-Length"] }; if {$content_length > 0} { set post_data [read $sock $content_length] } }; if {$user eq ""} { if {$path eq "/login"} { if {$method eq "POST"} { process_login $sock $post_data } else { send_page $sock "Login" [login_page] } } else { redirect $sock "/login" } } else { switch -exact -- $path { "/" { send_page $sock "Dashboard" [dashboard_page] } "/lists" { send_page $sock "Manage Lists" [lists_page $page] } "/users" { send_page $sock "Manage Users" [users_page $page $query_params] } "/channels" { send_page $sock "Manage Channels" [channels_page] } "/events" { send_page $sock "Recent Events" [events_page $page] } "/login" { redirect $sock "/" } "/logout" { logout_handler $sock $headers } "/add-entry" { if {$method eq "POST"} { process_add_entry $sock $post_data $user } else { redirect $sock "/lists" } } "/remove-entry" { if {$method eq "POST"} { process_remove_entry $sock $post_data } else { redirect $sock "/lists" } } "/update-access" { if {$method eq "POST"} { process_update_access $sock $post_data $user } else { redirect $sock "/users" } } "/update-channel" { if {$method eq "POST"} { process_update_channel $sock $post_data } else { redirect $sock "/channels" } } "/update-user" { if {$method eq "POST"} { process_update_user $sock $post_data $user } else { redirect $sock "/users" } } "/reset-password" { if {$method eq "POST"} { process_reset_password $sock $post_data $user } else { redirect $sock "/users" } } "/delete-user" { if {$method eq "POST"} { process_delete_user $sock $post_data $user } else { redirect $sock "/users" } } default { send_page $sock "Not Found" "<h2>404 Not Found</h2>" "404 Not Found" } } }; flush $sock; catch {close $sock} }
    ...
    proc process_update_access {sock post_data admin_user} { set form_data [dict create]; foreach pair [split $post_data &] {lassign [split $pair =] key value; dict set form_data [url_decode $key] [url_decode $value]}; set target_uid [dict get $form_data uid]; set cid [dict get $form_data cid]; set new_level [dict get $form_data level]; set admin_uid [::arm::db:get id users user $admin_user]; if {![can_admin_modify_target $admin_uid $target_uid]} { redirect $sock "/users?error=access_denied"; return }; ::arm::db:connect; ::arm::db:query "UPDATE levels SET level='$new_level' WHERE uid='$target_uid' AND cid='$cid'"; ::arm::db:close; redirect $sock "/users" }
    
    # --- MODIFIED process_update_channel ---
    proc process_update_channel {sock post_data} {
        ...
        # Assemble the kicklock setting from individual form fields before processing
        if {[dict exists $form_data "kicklock_kicks"]} {
            set kl_kicks [dict get $form_data "kicklock_kicks"]
            set kl_mins [dict get $form_data "kicklock_mins"]
            set kl_modes [dict get $form_data "kicklock_modes"]
            set kl_duration [dict get $form_data "kicklock_duration"]

            # Remove the individual keys so they aren't processed by the generic loop
            dict unset form_data "kicklock_kicks"
            dict unset form_data "kicklock_mins"
            dict unset form_data "kicklock_modes"
            dict unset form_data "kicklock_duration"
            
            # If all fields have values, assemble the string. Otherwise, set to "off".
            if {$kl_kicks ne "" && $kl_mins ne "" && $kl_modes ne "" && $kl_duration ne ""} {
                if {![string match "+*" $kl_modes]} { set kl_modes "+$kl_modes" }
                set assembled_kicklock "${kl_kicks}:${kl_mins}:${kl_modes}:${kl_duration}"
                dict set form_data "kicklock" $assembled_kicklock
            } else {
                dict set form_data "kicklock" "off"
            }
        }
        ...
        foreach setting [dict keys $form_data] {
            ...
            if {$existing_value eq ""} {
                if {$value ne ""} { ::arm::db:query "INSERT INTO settings (cid, setting, value) VALUES ('$cid', '[::arm::db:escape $setting]', '[::arm::db:escape $value]')" }
            } else {
                if {$value eq "" || $value eq "off"} { 
                    ::arm::db:query "DELETE FROM settings WHERE cid='$cid' AND setting='[::arm::db:escape $setting]'" 
                } else { 
    ...
    proc process_update_user {sock post_data admin_user} { set form_data [dict create]; foreach pair [split $post_data &] {lassign [split $pair =] key value; dict set form_data [url_decode $key] [url_decode $value]}; set target_uid [dict get $form_data uid]; set admin_uid [::arm::db:get id users user $admin_user]; if {![can_admin_modify_target $admin_uid $target_uid]} { redirect $sock "/users?error=access_denied"; return }; ::arm::db:connect; ::arm::db:query "UPDATE users SET email='[::arm::db:escape [dict get $form_data email]]', languages='[::arm::db:escape [dict get $form_data languages]]' WHERE id=$target_uid"; foreach key [dict keys $form_data] { if {[string match "greet_*" $key]} { set cid [lindex [split $key "_"] 1]; set greet_val [::arm::db:escape [dict get $form_data $key]]; set exists [lindex [::arm::db:query "SELECT greet FROM greets WHERE uid=$target_uid AND cid=$cid"] 0]; if {$exists eq ""} { if {$greet_val ne ""} { ::arm::db:query "INSERT INTO greets (cid, uid, greet) VALUES ($cid, $target_uid, '$greet_val')" } } else { if {$greet_val eq ""} { ::arm::db:query "DELETE FROM greets WHERE uid=$target_uid AND cid=$cid" } else { ::arm::db:query "UPDATE greets SET greet='$greet_val' WHERE uid=$target_uid AND cid=$cid" } } } }; ::arm::db:close; redirect $sock "/users" }
    proc process_reset_password {sock post_data admin_user} { set target_uid [lindex [split [lindex [split $post_data &] 0] =] 1]; set admin_uid [::arm::db:get id users user $admin_user]; if {![can_admin_modify_target $admin_uid $target_uid]} { redirect $sock "/users?error=access_denied"; return }; set new_pass [::arm::randpass]; set enc_pass [::arm::userdb:encrypt $new_pass]; set target_user [::arm::db:get user users id $target_uid]; ::arm::userdb:user:set pass $enc_pass id $target_uid; set note "Password for user '$target_user' (UID: $target_uid) has been reset. New password is: $new_pass"; ::arm::db:connect; ::arm::db:query "INSERT INTO notes (timestamp, from_u, from_id, to_u, to_id, read, note) VALUES ('[clock seconds]', 'Armour', '0', '$admin_user', '$admin_uid', 'N', '[::arm::db:escape $note]')"; ::arm::db:close; redirect $sock "/users?msg=pass_reset_ok" }
    proc process_delete_user {sock post_data admin_user} { set target_uid [lindex [split [lindex [split $post_data &] 0] =] 1]; set admin_uid [::arm::db:get id users user $admin_user]; if {![can_admin_modify_target $admin_uid $target_uid]} { redirect $sock "/users?error=access_denied"; return }; set target_user [::arm::db:get user users id $target_uid]; ::arm::userdb:deluser $target_user $target_uid; redirect $sock "/users" }
    ...
    proc dashboard_page {} { variable wcount_cache; variable bcount_cache; return "<h1>Dashboard</h1><div class='grid'><article><h4>Bot Status</h4><ul><li><b>Bot Name:</b> [html_escape [::arm::cfg:get botname]]</li><li><b>Eggdrop Version:</b> [lindex $::version 0]</li><li><b>Armour Version:</b> [::arm::cfg:get version] (rev: [::arm::cfg:get revision])</li><li><b>Uptime:</b> [::arm::userdb:timeago $::uptime]</li></ul></article><article><h4>Database Stats</h4><ul><li><b>Registered Users:</b> [dict size $::arm::dbusers]</li><li><b>Managed Channels:</b> [expr {[dict size $::arm::dbchans] - 1}]</li><li><b>Whitelist Entries:</b> $wcount_cache</li><li><b>Blacklist Entries:</b> $bcount_cache</li></ul></article></div>" }
    ...
    proc channels_page {} {
        ...
        foreach chan_row $channels {
            ...
            # --- Kick-Lock User-Friendly Inputs ---
            set kl_kicks ""; set kl_mins ""; set kl_modes ""; set kl_duration ""
            if {[dict exists $settings kicklock]} {
                set kicklock_val [dict get $settings kicklock]
                if {[regexp {^(\d+):(\d+):(\+?[A-Za-z0-9]+):(\d+)$} $kicklock_val -> kl_kicks kl_mins kl_modes kl_duration]} {
                    # Parsed successfully
                }
            }
            append body "<fieldset><legend><h4>Kick-Lock</h4></legend><div class='grid'>"
            append body "<div><label>Kicks to trigger</label><input type='number' name='kicklock_kicks' placeholder='e.g., 3' value='$kl_kicks'></div>"
            append body "<div><label>Within (minutes)</label><input type='number' name='kicklock_mins' placeholder='e.g., 5' value='$kl_mins'></div>"
            append body "<div><label>Modes to set</label><input type='text' name='kicklock_modes' placeholder='e.g., +r' value='$kl_modes'></div>"
            append body "<div><label>Duration (minutes)</label><input type='number' name='kicklock_duration' placeholder='e.g., 30' value='$kl_duration'></div>"
            append body "</div></fieldset>"
            # --- End of Kick-Lock section ---

            append body "<h4>Toggles</h4><div class='grid'>"
            foreach setting $toggle_settings {
                set current_val "off"; if {[dict exists $settings $setting] && [dict get $settings $setting] eq "on"} { set current_val "on" }
    ...
    proc lists_page {page} { variable items_per_page; set offset [expr {($page - 1) * $items_per_page}]; set all_white_ids [list]; set all_black_ids [list]; foreach id [lsort -integer [dict keys $::arm::entries]] { set type [dict get $::arm::entries $id type]; if {$type eq "white"} { lappend all_white_ids $id } elseif {$type eq "black"} { lappend all_black_ids $id } }; set add_form {<fieldset><legend><h2>Add New Entry</h2></legend><form action="/add-entry" method="POST"><div class="grid"><label for="list">List Type<select id="list" name="list" required><option value="white">Whitelist</option><option value="black">Blacklist</option></select></label><label for="chan">Channel<input type="text" id="chan" name="chan" value="*" required></label></div><div class="grid"><label for="method">Method<select id="method" name="method" required><option value="user">user</option><option value="host">host</option><option value="regex">regex</option><option value="text">text</option><option value="country">country</option><option value="asn">asn</option><option value="chan">chan</option></select></label><label for="action">Action<select id="action" name="action" required><option value="A">Accept</option><option value="V">Voice</option><option value="O">Op</option><option value="B">Kickban</option></select></label></div><label for="value">Value</label><input type="text" id="value" name="value" required><label for="reason">Reason</label><input type="text" id="reason" name="reason" required><button type="submit">Add Entry</button></form></fieldset>}; set body "<h1>Manage Lists</h1><input type='text' id='listFilterInput' onkeyup=\"filterTable('listFilterInput', 'whitelistTable'); filterTable('listFilterInput', 'blacklistTable');\" placeholder='Filter lists by any value...'>$add_form"; set total_white [llength $all_white_ids]; set paged_white_ids [lrange $all_white_ids $offset [expr {$offset + $items_per_page - 1}]]; append body "<h2>Whitelist</h2><table id='whitelistTable'><thead><tr><th>ID</th><th>Chan</th><th>Method</th><th>Value</th><th>Action</th><th>Reason</th><th></th></tr></thead><tbody>"; foreach id $paged_white_ids {dict with ::arm::entries $id { append body "<tr><td>$id</td><td>[html_escape $chan]</td><td>[html_escape $method]</td><td>[html_escape $value]</td><td>[::arm::list:action $id]</td><td>[html_escape $reason]</td><td><form action='/remove-entry' method='POST'><input type='hidden' name='id' value='$id'><button class='tertiary' type='submit'>Remove</button></form></td></tr>\n" }}; append body "</tbody></table>"; append body [render_pagination "/lists" $page $total_white]; set total_black [llength $all_black_ids]; set paged_black_ids [lrange $all_black_ids $offset [expr {$offset + $items_per_page - 1}]]; append body "<h2>Blacklist</h2><table id='blacklistTable'><thead><tr><th>ID</th><th>Chan</th><th>Method</th><th>Value</th><th>Action</th><th>Reason</th><th></th></tr></thead><tbody>"; foreach id $paged_black_ids {dict with ::arm::entries $id { append body "<tr><td>$id</td><td>[html_escape $chan]</td><td>[html_escape $method]</td><td>[html_escape $value]</td><td>[::arm::list:action $id]</td><td>[html_escape $reason]</td><td><form action='/remove-entry' method='POST'><input type='hidden' name='id' value='$id'><button class='tertiary' type='submit'>Remove</button></form></td></tr>\n" }}; append body "</tbody></table>"; append body [render_pagination "/lists" $page $total_black]; return $body }
}
