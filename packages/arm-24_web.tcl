...
# This version includes comprehensive permission checks and an optimized user management page.
...
namespace eval ::arm::web {
    ...
    proc can_admin_modify_target {admin_uid target_uid} {
        ...
        foreach level_row $admin_levels { if {[lindex $level_row 0] > $admin_max_level} { set admin_max_level [lindex $level_row 0] } }
        set target_levels [::arm::db:query "SELECT level FROM levels WHERE uid=$target_uid"]
        set target_max_level 0
        foreach level_row $target_levels { if {[lindex $level_row 0] > $target_max_level} { set target_max_level [lindex $level_row 0] } }
        ::arm::db:close
    ...
    proc users_page {page query_params} {
        ...
        # --- 1. Get the list of users for the current page ---
        ...
        set user_ids [list]
        foreach user_row $users { lappend user_ids [lindex $user_row 0] }

        # --- 2. Fetch all required data for these users in batch queries ---
        set levels_by_user [dict create]
        set greets_by_user [dict create]
        set chan_names [dict create]

        if {[llength $user_ids] > 0} {
            set user_id_list [join $user_ids ","]
            # Fetch all levels for the users on this page
            set all_levels [::arm::db:query "SELECT uid, cid, level FROM levels WHERE uid IN ($user_id_list) ORDER BY uid, cid"]
            foreach level_row $all_levels {
                lassign $level_row l_uid l_cid l_level
                dict lappend levels_by_user $l_uid [list $l_cid $l_level]
            }

            # Fetch all greets for the users on this page
            set all_greets [::arm::db:query "SELECT uid, cid, greet FROM greets WHERE uid IN ($user_id_list)"]
            foreach greet_row $all_greets {
                lassign $greet_row g_uid g_cid g_greet
                dict set greets_by_user $g_uid $g_cid $g_greet
            }
        }
        
        # Pre-fetch all channel names to avoid lookups in the loop
        set all_channels [::arm::db:query "SELECT id, chan FROM channels"]
        foreach chan_row $all_channels {
            lassign $chan_row c_id c_name
            dict set chan_names $c_id $c_name
        }
        
        ::arm::db:close

        # --- 3. Render the page using the pre-fetched data ---
        ...
        foreach user_row $users {
            ...
            if {[dict exists $levels_by_user $uid]} {
                foreach level_pair [dict get $levels_by_user $uid] {
                    lassign $level_pair cid level
                    set chan_name [dict get $chan_names $cid]
                    set greet ""
                    if {[dict exists $greets_by_user $uid $cid]} {
                        set greet [dict get $greets_by_user $uid $cid]
                    }
                    append body "<tr><td><b>[html_escape $chan_name]</b></td><td><form style='display:inline-block; margin:0;' action='/update-access' method='POST'><input type='hidden' name='uid' value='$uid'><input type='hidden' name='cid' value='$cid'><input type='number' name='level' value='$level' style='width: 70px;'><button class='tertiary' type='submit'>Save</button></form></td><td><input type='text' name='greet_${cid}' value='[html_escape $greet]'></td></tr>"
                }
            }
    ...
    # --- END: OPTIMIZED users_page ---

    proc channels_page {} { set body "<h1>Channel Management</h1><table id='channelsTable'><thead><tr><th>Channel Details</th></tr></thead><tbody>"; set toggle_settings {strictop strictvoice autotopic operop trakka quote}; ::arm::db:connect; set channels [::arm::db:query "SELECT id, chan FROM channels WHERE chan != '*' ORDER BY chan"]; foreach chan_row $channels { lassign $chan_row cid chan; set settings [dict create]; set setting_rows [::arm::db:query "SELECT setting, value FROM settings WHERE cid=$cid"]; foreach setting_row $setting_rows {lassign $setting_row key val; dict set settings $key $val}; append body "<tr><td><details><summary><b>[html_escape $chan]</b></summary>"; append body "<form action='/update-channel' method='POST' style='margin-top: 1em; border-left: 2px solid #ccc; padding-left: 1em;'><input type='hidden' name='cid' value='$cid'>"; set current_mode ""; if {[dict exists $settings mode]} { set current_mode [dict get $settings mode] }; set url_val ""; if {[dict exists $settings url]} { set url_val [html_escape [dict get $settings url]] }; set desc_val ""; if {[dict exists $settings desc]} { set desc_val [html_escape [dict get $settings desc]] }; set kicklock_val ""; if {[dict exists $settings kicklock]} { set kicklock_val [dict get $settings kicklock] }; append body "<div class='grid'><div><label for='mode_$cid'>Mode</label><select id='mode_$cid' name='mode'>"; foreach mode_option {on off secure} {set selected [expr {$current_mode eq $mode_option ? "selected" : ""}]; append body "<option value='$mode_option' $selected>[string totitle $mode_option]</option>"}; append body "</select></div><div><label for='kicklock_$cid'>Kick-Lock <small>(kicks:mins:+modes:duration)</small></label><input type='text' id='kicklock_$cid' name='kicklock' placeholder='e.g., 3:5:+r:30' value='$kicklock_val'></div></div>"; append body "<label for='url_$cid'>URL</label><input type='text' id='url_$cid' name='url' value='$url_val'><label for='desc_$cid'>Description</label><input type='text' id='desc_$cid' name='desc' value='$desc_val'>"; append body "<h4>Toggles</h4><div class='grid'>"; foreach setting $toggle_settings { set current_val "off"; if {[dict exists $settings $setting] && [dict get $settings $setting] eq "on"} { set current_val "on" }; append body "<div><label for='${setting}_$cid'>[string totitle $setting]</label><select id='${setting}_$cid' name='$setting'>"; append body "<option value='on' [expr {$current_val eq "on" ? "selected" : ""}]>On</option><option value='off' [expr {$current_val eq "off" ? "selected" : ""}]>Off</option></select></div>" }; append body "</div><button type='submit' style='margin-top:1em;'>Update [html_escape $chan]</button></form></details></td></tr>" }; ::arm::db:close; append body "</tbody></table>"; return $body }
